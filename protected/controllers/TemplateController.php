...
class TemplateController extends Controller {
    ...
    public function actionForm($id = null, $template_type_id = null) {
        ...
        if (!empty($_POST["Template"])) {
            ...
            if ($model->image) {
                 $attach_src = Yii::app()->basePath . '/../docs/' . $fecha.'.'.$model->image->getExtensionName(); //. '_' . $model->$imageAttrName;
                 $model->image->saveAs($attach_src);
                 $model->image = $fecha.'.'.$model->image->getExtensionName();// . '_' . $model->$imageAttrName;
            }
